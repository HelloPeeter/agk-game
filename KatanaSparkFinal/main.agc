...
dim enemyX[40]
enemyY=samuraiY-3
enemyRunSpeed=8 //Velocidade dos inimigos
enemyRight=1
enemyLeft=0
enemyCount=18 // --> Quantidade de inimigos

vol=10 //Seta uma variavel padrao para o Volume

//Variveis de menu/tempo
timerEnd=0
menu=0
startRoundSpeech=0
startRound=0
...
do
	if(menu=0)
		...
		gosub set_scene_1
		if(GetSpritePlaying(menuSprite)=0)
			playSprite(menuSprite, 13, 1, 1, 26)
		endif
		soundLoop(screenOst, vol)	
		if(GetSpriteCollision(mouse, button) and GetPointerPressed())
			//Começando o jogo e resetando as variaveis!
			menu = 1
			setBackground_1 = 1
			timerEnd=0
			samuraiLife = 5
			...
			score = 0
			stopSound(screenOst)
			setSpriteSize(bar[1], 30*5, 52)
		endif		
	else
		gosub set_scene_1	
		if(samuraiLife>0)
			if(playSong=0)
				soundLoop(inGameOst, vol-5)
			else	
				stopSound(inGameOst)
			endif		
			gosub start_game
