...
SetSpritePosition(bubbleSprite[0], samuraiX, samuraiY-32)
SetSpritePosition(startSprite[0], samuraiX+12, samuraiY-32)
vol = 10
	...
	//ATTACK_1 - F
	if(key_f and GetSpritePlaying(6)=0 and getSpritePlaying(7)=0 and getSpritePlaying(10)=0) //Se a Tecla F é pressionada
	...
	if(playSong and GetSpriteCurrentFrame(10)=22 or keep=1 and PlaySong) //Mantem a animaçao em Looping
		...
		if(getSpritePlaying(bubbleSprite[0])=0)
			SetSpriteVisible(bubbleSprite[0], 1)
			PlaySprite(bubbleSprite[0], 3, 1, 7, 9)
		endif	
	endif
	
	//Para a musica se alguma açao for feita	
